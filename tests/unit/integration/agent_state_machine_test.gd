...
func test_same_state_transition_does_not_emit() -> void:
	...
	# Another tool_use response — should stay working, no emission.
	asm._on_agent_response_received("a1", AsmFixtures.payload_tool_use())
	assert_eq(emit_count, 0, "Same-state transition does not emit signal")
	asm.queue_free()
...
func test_task_completed_emits_from_working_to_completed() -> void:
	...
	assert_eq(emit_count, 1, "task_completed emits on working → completed")
	asm.queue_free()
